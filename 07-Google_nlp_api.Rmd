...
```{r cache-google-nlp, include=FALSE} 
...
file_R <- knitr::purl(input = "07-Google_nlp_api.Rmd") # extract R-code
source(file_R)
p <- NCmisc::list.functions.in.file(file_R) # chec
file.remove(file_R)
packages <- gsub("package:", "", names(p)[str_detect(names(p), "^package:")])[-1]
dput(packages)
```

```{r}
library(pacman)
p_load(c("dplyr", "ggplot2", "ggwordcloud", "purrr", "tidyr", "utils"), 
       character.only = TRUE)
```
