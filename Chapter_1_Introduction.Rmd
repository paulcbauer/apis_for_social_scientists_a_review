...
# Introduction
The project *APIs for Social Scientists: A Review* is an outcome of the seminar *Computational Social Science* (CSS) taught at the University of Mannheim. While teaching the seminar we had trouble finding short reviews of APIs with quick R-code examples. Fortunately, almost everyone participating in the seminar was motivated enough to write a quick API review. The current document is available in PDF on OSF and as HTML under the following link. Hopefully, our ressource will be helpful future students that start to dive into CSS.
...
## Authentication
A lot of the APIs require that you authenticate with the API provider. The current script is contains R chunks (that are not visible) for authentication. As a consequence we can not make the corresponding [github repository public](https://github.com/paulcbauer/apis_for_social_scientists_a_review).
