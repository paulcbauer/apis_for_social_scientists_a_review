...
The result is an HTTP response with JSON formatted data which contains the predicted gender, the prediction accuracy estimate and the count of entries which informed the prediction. For the example requests above, the API responds with:
...
This tells us that for the requested name "sasha", the gender was predicted as male but only with probability 0.51. This makes sense since this name is considered a unisex name in many countries. The prediction is based on 13219 samples in the database, which seems very solid.
...
Interestingly, only the Latinized forms of Sasha seem to be available in the database. Neither the Cyrillic form Саша, nor other non-Latin forms like Saša return results. (TODO: check this again using genderizer package)
