...
# setting cache to TRUE here might allows that the API call (e.g., line 140) has to be run only once and after that the chunk can be set to eval=F so that the API is not used each single time knitting index.rmd -> doesnt work up until now
```
...
Here we describe how a simple API call can be made within the Google Cloud Platform environment via the Google Cloud Shell:
...
The input (i.e., text data) one provides to the API most often will go beyond a single word or sentence. The most convenient way which also produces the most insightful and structured results (on which you can directly perform further analysis on) are achieved when using the [‘googleLanguageR’ R package]("https://cran.r-project.org/web/packages/googleLanguageR/index.html") - a package directly corresponding to the API.
...
syntax_analysis <- gl_nlp(df$text, nlp_type = "analyzeSyntax") # taucht in cache nicht auf
```

```{r include=F, echo=F, eval=F}
save(syntax_analysis, file="./Chapter_2/Chapter_2_df.RData") # could be removed if cache works
```

```{r echo=F}
load("./Chapter_2/Chapter_2_df.RData") # could be removed if cache works
```
